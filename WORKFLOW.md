...
| task branch             | 从目标分支当前提交创建，承载本任务完整生命周期的唯一分支                    |
...
- 实施者完成 task branch 后，协调员检查目标分支。目标分支已经前进时，协调员根据仓库 Git 策略在同一 task branch 上执行 rebase 或 merge；同步后的 task branch 进入双重验证和 Review。
...
| Verification | task branch 已完成目标分支检查 | 实施者先验证最终 task branch，协调员随后独立验证；双方均通过；委派证据已回写；外部资源已归类                                    |
| Review       | Verification Gate 通过         | task branch 在 Review 期间已冻结；所有阻塞 finding 已由对应审查者关闭                                                           |
| Integration  | Review Gate 通过               | 目标分支仍是被审 task branch 的祖先；证据语义核对通过；`check --final` 通过；task branch 已 fast-forward 集成；非预期资源已清理 |
...
- 协调员只在实施者完成 task branch 的实现和验证后检查目标分支。同步改变 task branch 时，实施者重新验证同步后的分支。
- Verification Gate 只有在实施者先验证最终 task branch、协调员随后独立验证，且双方均通过后才放行；双方通过前不得进入 Review。
- 任一 Gate 未通过时停在当前阶段，或返回表中能够解除缺口的前序阶段。禁止跨过 Gate、跳过有效测试、伪造证据或把未运行验证报告为通过。
- Review 期间冻结 task branch。
- 审查者接受后，只允许实施者提交对应任务账本的 Review 结论和状态；task branch 的其他文件变化会使 Review Gate 失效。
- 目标分支前进会使 Integration Gate 失效。协调员重新同步 task branch；实施者和协调员依次验证后，由同一审查者复审。
...
- [ ] task branch 是否在首次任务专属写入前从明确的目标分支创建，并承载本任务全部改动？
- [ ] task worktree、task branch、implementation terminal 和 Review 上下文是否唯一且可定位？
...
- [ ] 主工作区和 task worktree 是否没有非预期残留？
- [ ] 任务完成前是否已完成自审？
- [ ] 未验证项和剩余风险是否已明确记录？

## 6. 最终报告

报告 task worktree、task branch、implementation terminal 和 Review 上下文，列出 `IMP → BEH/VER → evidence` 映射、目标分支同步方式、各 Gate（Ready、Child、Verification、Review、Integration）证据、Code Review 结论、Integration Gate 证据核对结论、Spike 或产品决策结果（如有）、外部资源收口、fast-forward 集成结果、未验证项和剩余风险。
