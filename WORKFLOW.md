...
flowchart TD
    ...
    Q1 -- 是 --> U1[Coordinator: 创建或绑定 Run 并创建 Review Task]
    U1 --> W2[Coordinator: worker-start 派发 reviewer 并立即进入等待循环]
    W2 --> L1[Coordinator: 当前回合有界滚动 check --wait 等待 FIFO Delivery]
    ...
    P0 --> S0
    S0 -- 是 --> W1[Coordinator: worker-start 派发 writer 并记录引用]
    W1 --> L1
    I1[writer lane 异步实施、提交并发送 worker_done 等正式事件] -. 事件写入 Run inbox .-> L1
    L1 --> L2{返回 oldest FIFO Delivery}
    L2 --> L3{该 Delivery 还有未处理 Message?}
    L3 -- 否 --> L9[事务尾部：完成全部 durable side effects、处置权威 settled worker、最后 ack]
    L3 -- 是 --> L4{Message 类型}
    L4 -- question --> L5[Coordinator: 通过 ask/reply 回答] --> L3
    L4 -- escalation --> L6[Coordinator: 回复、阻塞、停止、重规划或创建替代 Task] --> L3
    L4 -- worker_done --> L7[Coordinator: 校验预期 Task 与对应 Dispatch attempt]
    L7 -- stale 或 rejected --> L3
    L7 -- 有效 --> R2{outcome}
    R2 -- failed 或 stopped --> F1[Coordinator: 记录失败，决定 retry / replacement / blocked / stop] --> L3
    R2 -- succeeded --> R3{Task role 或处理阶段}
    R3 -- implementation / remediation --> L8a[仓库候选验证：Spec drift、scope、final SHA、HEAD 与 clean]
    L8a -- 候选接受 --> L3
    L8a -- 候选拒绝 --> F2[Coordinator: 记录原因、创建 remediation / replacement Task] --> L3
    R3 -- review --> L8b[复核 findings 与 worktree 未变化]
    L8b -- 有阻塞 finding --> F2 --> L3
    L8b -- 无阻塞 --> L3
    L9 --> L10{处理后的工作流状态}
    L10 -- 仍有 expected unsettled Dispatch --> L1
    L10 -- implementation 接受 --> C3[Coordinator: 同步 target branch]
    C3 --> C4[自动化检查与必要 manual/live verification]
    C4 --> C5[复核 requirement source 与适用 Spec、记录 Review SHA、创建 Review Task 并确认 clean]
    C5 --> W2
    L10 -- retry / replacement / remediation 决策 --> S0
    L10 -- review 无阻塞且 target 前进 --> C9[Coordinator: 重新同步并运行受影响检查与验证]
    C9 --> C5
    L10 -- review 无阻塞且 target 未前进 --> C10{退出条件满足?}
    L10 -- failed / blocked / stop 无继续路径 --> C12[Coordinator: 确认 Delivery 已处理、worker 已处置后报告]
    C10 -- 否 --> C2
...
- 变化影响当前目标、行为、公共契约、权限、状态模型或验证规则：结算受影响 Dispatch，使用 Orca 支持的 Task status；在 result/reason 中记录 `spec_changed` 与变化原因（`spec_changed` 只能写入 result/reason，不是 Task status，不得写成自定义 status），再创建替代 Task。
- 仍符合新版 requirement/Spec 的现有 commit 或 diff 可作为替代 Task 输入，不机械推倒重做。

Coordinator 区分三个独立判断：Message 是否属于预期 Task 与对应 Dispatch attempt；`worker_done` outcome 是 succeeded 还是 failed；succeeded 结果产生的 Git candidate 是否可接受。stale、rejected 或重复但不属于当前预期 attempt 的消息不得成为候选依据，也不触发 worker 处置。

对有效 succeeded 结果，只接受同时满足以下条件的完成结果：`taskId` 对应当前 Task；`dispatchId` 对应该 Task 的对应 Dispatch attempt（有效 `worker_done` 后 runtime 状态可以已离开 active/dispatched 并进入终态，不假定 outcome 与具体状态名称的映射，应结合 Message、Task/Dispatch 当前 runtime 状态与预期 attempt 判断）；报告的 final SHA 可定位；修改文件属于声明的 write scope（或越界原因已被 Coordinator 接受）；writer 已交还相关 ownership。

对有效 failed/stopped 结果：记录失败并按第 6 节决定 retry、replacement、blocked 或 stop；不要求也不假定 candidate final SHA；同样在 ack 前完成 worker 处置。
...
5. `worker_done` 先区分三种判断：Message 是否属于预期 Task 与对应 Dispatch attempt（排除 stale、rejected 或错误 attempt）；outcome 是 succeeded 还是 failed；succeeded 产生的 Git candidate 是否可接受（第 5 节）。有效结果由 Orca runtime 按合同自动结算 Task 与 Dispatch，不追加手动结算；stale、rejected 或错误 attempt 不得作为候选依据，也不触发 worker 处置。
6. outcome 为 failed/stopped：记录失败，按 runtime 证据与当前 Skill 决定 retry、replacement、blocked 或 stop；不要求也不假定 candidate final SHA；同样在 ack 前完成 worker 处置。
7. outcome 为 succeeded：执行仓库候选验证（第 5 节）；候选拒绝时记录原因并创建 remediation / replacement Task；无论候选接受还是拒绝，都在 ack 前完成该 worker 的处置。
8. 每个权威 `worker_done`（succeeded 或 failed）之后、ack 之前，决定该 worker terminal 的归属：同一 agent 有立即 follow-up Task 时按当前 Skill 复用；用户明确要求保留时执行 worker-retain；其余执行 worker-release。stale、rejected 或错误 attempt 的消息不得触发清理。
9. 事务尾部：完成该 Delivery 的全部 durable side effects（回复、Task 创建或结算决策、worker 处置、checkpoint 更新）后，ack 是事务最后一步；随后若仍有 expected unsettled Dispatch，继续下一次有界 wait。
10. timeout、空结果、heartbeat、可见 terminal 活动或 TUI idle 都不等于完成或失败；只要仍有 expected unsettled Dispatch，就继续等待，除非 runtime 证明 worker 失败、停止或丢失，或用户明确停止。
11. 只有全部 expected Dispatch 已结算且满足 Merge 条件（第 12 节）时，Coordinator 才结束监督循环并进入集成。
...
- 只在 context compact 前、ownership 交接前、关键决策、阻塞或重规划时、重要 Delivery 处理完成或显式恢复校准后更新。若 context 中断发生在 Delivery 处理与 ack 之间，只在现有 `Decisions / blockers / unverified items` 字段记录最小事务摘要：Delivery ID、已处理 Message IDs、已完成 durable side effects 与 ack 状态；恢复时先核对这些副作用，避免重复回复、重复建 Task、重复集成或错误清理，不新增 checkpoint 字段。恢复顺序：读取 handoff 与当前 worktree checkpoint → 绑定或接管正确 Run → 读取 Git 与 Orca runtime → 读取 requirement source 与适用 Spec → 修正过期摘要 → 读取该 Run 的 oldest unacked FIFO Delivery，将重放 Message 与 checkpoint 中已记录的 durable side effects 对账 → 完成尚未完成的副作用（回复、Task 创建、worker 处置）→ 最后 ack → 仍有 expected unsettled Dispatch 时重新进入有界 `check --wait` → 从 Next action 继续。
...
- [ ] 存在 Orca Task 时，其 requirement source、适用 Spec references、write scope、required checks 和 dependencies 已明确；低风险直接路径以当前工作的语义要求为准。
...
- [ ] 如存在 required Dispatch，其 expected attempts 已结算并完成候选判断，阻塞 question/escalation 已解决，writer ownership 已归还，没有仍会修改最终候选 HEAD 的 active Dispatch。
