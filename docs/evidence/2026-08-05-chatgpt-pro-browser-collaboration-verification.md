...
| VER-004 | ⚠️ 部分   | 既有正常路径已证明目标 assistant、Stop/Copy、页面内 clipboard 拦截、Copy 与 response.md 的 4,121 字节相等、空 artifact 及重复 wait 幂等；修复后 39 项真实执行 page-function fixture 已覆盖预 reload target 持久绑定、T1→T2、post-reload stable baseline/fingerprint、内容变化、local key、stall grace、延迟成功 reload、reload deadline/回滚，但卡滞指示 live 场景尚未在 Host A 同一 turn 重跑，不能判定完整通过。 |
...
| `pnpm test`                                                                 | ✅ 8 个 test file、111 个测试通过                         |
| `pnpm exec vitest --run tests/state-concurrency.test.ts`                    | ✅ 10 个真实子进程恢复与并发测试通过                      |
| `pnpm exec vitest --run tests/browser.test.ts`                              | ✅ 45 个浏览器边界与可执行页面 fixture 测试通过           |
...
`spec-tasks diff` 返回 `specChanged=false`、`contextChanged=false`、无 impacted task。`implemented` 只证明实现提交与相关检查已取得，可满足下游实施依赖；它不代表全量 VER 或 Review 已通过。IMP-002、IMP-005、IMP-007 保持 `done`；IMP-004、IMP-008 在 `2c17644` 后保持 `implemented`，等待修复后 live VER-004 与独立 Review；IMP-001、IMP-003、IMP-006 的既有缺口保持不变。最终 `check --final` 退出 1，并逐项报告 IMP-001、IMP-003、IMP-004、IMP-006、IMP-008 必须为 `done` 或 `cancelled`。
