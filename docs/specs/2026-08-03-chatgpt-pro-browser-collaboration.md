...
| VER-010 | BEH-001–BEH-009 的确定性实现       | 依赖已安装，目标代码和测试存在                            | 在最后一次相关修改后运行 `pnpm format:check`、`pnpm lint`、`pnpm typecheck`、`pnpm test`、`pnpm collab -- help`；从无 `package.json` 的临时宿主目录用绝对路径执行 Skill CLI `help` | 每条命令退出码为 0；安装后入口不依赖宿主 package manifest；测试输出能追溯对应 `BEH-*`              | 不证明真实 ChatGPT Web 行为；不能替代 VER-001–VER-009 |
