...
- **可观察行为**：Collab 打开一次交互式登录流程，并在用户完成登录后保留可供后续任务复制的本地认证源；`setup` 完成后关闭本次设置使用的浏览器。
...
- **可观察行为**：Collab 返回唯一 `taskId`，为该任务创建认证工作副本、独立浏览器进程、独立会话目录和一张新的 ChatGPT 对话页面。首次成功发送时，任务固定绑定该页面建立的 conversation；任务不复用其他任务的浏览器进程、可写 profile 或 conversation。
- **验收条件**：任意两个活动任务具有不同的 `taskId`、浏览器进程、认证工作副本和会话目录，首次发送后具有不同 conversation；启动新任务不因同一项目或其他未关闭任务而拒绝。
- **状态**：已确认。
...
- **可观察行为**：Collab 终止该任务的浏览器进程并删除该任务的可丢弃认证工作副本，同时保留 BEH-007 的 transcript。`close` 不归档或删除 Web conversation，也不修改用户仓库。
- **验收条件**：浏览器进程和认证工作副本不再存在；transcript 仍可读取；Web conversation 保持原状态；重复 `close` 不产生额外副作用。
- **状态**：已确认。
...
- 认证源和任务认证副本只保存在本机，不作为附件上传。认证源持续保留供新任务使用；任务副本按 BEH-008 删除。本版不新增 Unix 权限模式合同。
...
- 目标仓库位于 `/Users/codeartz/workspaces/chatgpt-pro`，本次复读时 HEAD 为 `ee2999d`；工作区包含本规格和一项与本规格不重叠的 `AGENTS.md` 并发修改，后者不属于本次规格修订。
...
- 当前 manifest 没有浏览器自动化或数据库 npm dependency。2026-08-03 执行 `npm view @playwright/cli version` 得到 latest `0.1.17`；官方 CLI 提供 named session、persistent/profile、upload、run-code 和 close（REF-001）。
...
- 新规格不会复用 `/Users/codeartz/workspaces/chatgpt-pro-collab` 的现有实现；目标仓库是本规格的唯一代码基线。

## 技术设计

### 技术基线

- CLI 使用 Node.js 原生 type stripping 直接执行 ESM TypeScript，不生成运行产物，不引入 Jiti、tsx 或其他运行时转译器。运行代码只允许可擦除 TypeScript 语法，类型正确性由独立 `typecheck` 保证（REF-003）。
- 浏览器边界固定为 `@playwright/cli@0.1.17`。`browser.ts` 通过参数数组调用 `npx -y @playwright/cli@0.1.17`，不经 shell 拼接，也不把 Playwright 写入项目 dependency。每个 task 使用独立 named session 和独立 profile 路径（REF-001）。
- 控制状态固定使用 Node 标准库 `node:sqlite` 的同步 `DatabaseSync`；不引入第三方 ORM 或数据库 package。SQLite 保存 task/turn 的协调状态，逐 turn 文件保存可审计正文（REF-002）。
- 除 `state.sqlite` 外，文件系统只保存认证数据、prompt、response 和 Playwright 运行产物。SQLite 与 transcript 职责分开，不用 JSON 元数据文件复制数据库状态。
...
| Browser boundary | `skills/chatgpt-pro-collab/scripts/browser.ts` | BEH-001–BEH-006、BEH-009                    | Playwright CLI 进程调用、session/profile、ChatGPT 页面动作、回复捕获与归档 |
| State store      | `skills/chatgpt-pro-collab/scripts/state.ts`   | BEH-002–BEH-009                             | `node:sqlite` schema、事务、task/turn 状态和并发写入                       |
| Artifact store   | `skills/chatgpt-pro-collab/scripts/session.ts` | BEH-001、BEH-003、BEH-004、BEH-007、BEH-008 | 数据目录、认证副本、prompt/response 原子写入和本地清理                     |
...
- `browser.ts` 使用固定命令前缀 `npx -y @playwright/cli@0.1.17 -s=<sessionName> --raw`；不同 task 不共享 session name、profile 或输出目录。
- `setup` 使用独立 setup session，通过 `open https://chatgpt.com/ --browser=chrome --headed --persistent --profile=<seedPath>` 完成人工登录，验证登录后执行 session `close`。`start` 从 seed 建立 task 的 auth copy，再以相同 `open` 合同和 task profile 启动 task session；复制机制仍由未决事项约束。
...
- ChatGPT 专属 selector、conversation identity、完成检测、回复 Copy 和 Archive 全部封装在 `browser.ts` 的 `run-code` 脚本中。回复捕获拦截当前页面 `navigator.clipboard` 的 Copy response 写入，不读取或写入操作系统全局剪贴板。
- Playwright session 自身是浏览器生命周期权威；SQLite 只记录 session name、profile 路径和已观察到的 conversation identity，不伪造浏览器仍存活。

### 命令与结果合同

| 命令      | 必要输入                                          | 成功结果                             | 主要失败                                                         |
| --------- | ------------------------------------------------- | ------------------------------------ | ---------------------------------------------------------------- |
| `setup`   | 无                                                | 可复制认证源已建立，设置浏览器已关闭 | 用户未完成登录；认证源无法生成                                   |
| `start`   | 无                                                | `taskId`                             | 未 setup；认证复制或浏览器启动失败                               |
...
| 表     | 主键          | 必要字段                                                                                                                      |
| ------ | ------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `task` | `id`          | `playwright_session`、`profile_dir`、`conversation_id`、`conversation_url`、`status`、`created_at`、`updated_at`、`closed_at` |
| `turn` | `task_id, id` | `status`、`prompt_path`、`attachments_json`、`response_path`、`error`、`created_at`、`updated_at`                             |

`task.status` 只允许 `active | closed | failed`；`turn.status` 只允许 `sending | pending | completed | failed | unknown-submission`。`turn.task_id` 使用外键关联 task，session name 与 profile path 分别唯一。所有时间使用 ISO 8601 UTC 字符串；附件数组按上传顺序编码为 JSON 字符串。
...
- 同一 task 的 CLI 调用通过 SQLite 状态门和同一个 Playwright named session 排序；不同 task 使用不同 named session、浏览器进程、可写认证副本和 turn 目录。
...
- BEH-008 的进程终止或认证副本删除出现部分失败时，保留可定位的残留路径并返回失败；不删除 transcript 来伪造清理完成。
- task 和 turn 标识必须不可碰撞；文件写入先落到同目录临时文件再原子替换元数据。已经完成的 `prompt.md` 和 `response.md` 不再覆盖。
...
| ------- | ---------------------------------- | --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| VER-001 | BEH-001                            | 可交互登录的 Pro 账号与干净认证目录                       | 使用固定 Playwright CLI 执行一次 `setup`，随后两次 `start`                                                                                                 | setup named session 已关闭；两个任务均为已登录状态且未再次请求登录                                 | 不证明认证长期不过期                                  |
| VER-002 | BEH-002                            | VER-001 通过                                              | 同时启动两个任务，检查 Playwright session、浏览器进程、认证副本和 session 目录，并分别完成首次发送                                                         | 两个不同 named session、浏览器 PID、profile、taskId、session 路径和 conversation                   | 不证明消息并发或回复隔离                              |
...
| VER-004 | BEH-004                            | 已发送包含长文本与代码块、可产生完整回复的 turn           | 执行 `wait`，将页面内拦截的 Copy response 与 `response.md` 逐字节比较，再重复 wait                                                                         | 内容一致；没有读取系统剪贴板；重复调用返回同一路径；旧 response 未覆盖                             | 只覆盖当次 Web UI 和测试内容                          |
...
| VER-008 | BEH-008                            | 活动任务且 Web conversation 已建立                        | 执行 `close` 两次并检查 Playwright session、本地目录与 Web                                                                                                 | named session、浏览器进程和 auth-copy 消失；SQLite/transcript 保留；Web 未归档；第二次无额外副作用 | 不证明操作系统能清理被外部程序锁定的文件              |
| VER-009 | BEH-009                            | 活动任务且 Web conversation 已建立                        | 执行 `archive`，检查指定与其他 conversation 及本地任务                                                                                                     | 仅指定 conversation 进入 Archive；进程和 transcript 保持；其他 conversation 不变                   | 只覆盖当次 ChatGPT Web Archive UI                     |
...
| VER-012 | 技术基线                           | Node.js v22.19.0、当前受支持 Node、npm/npx 和网络可用     | 分别在最低与当前 Node 执行版本检查、`node:sqlite` DatabaseSync 内存库 smoke test 和最小 `.ts` 入口 smoke test；执行 `npx -y @playwright/cli@0.1.17 --help` | 两个 Node 版本的 smoke test 与固定 Playwright CLI 检查退出码均为 0                                 | 不证明登录、profile 复制或页面 selector               |
...
| 选择                      | 来源                                               | 状态   | 理由                                                                                           | 影响                                                                   |
| ------------------------- | -------------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| 轻量浏览器协作通道        | 用户 2026-08-03 要求从零重做                       | 已确认 | 旧方向把仓库安全门、集成和生命周期混入传输层，持续阻断正常协作；核心应只剩合作协议和浏览器操作 | BEH-001–BEH-009、产品边界、技术设计                                    |
| 每 task 独立浏览器进程    | 用户在重做讨论中选择真实并发与独立进程             | 已确认 | 多任务必须能够同时等待和操作，不能由共享浏览器或串行控制互相影响                               | BEH-002、BEH-006、VER-002、VER-006                                     |
| 宿主显式提供输入          | 用户在重做讨论中选择 promptPath 与 attachmentPaths | 已确认 | Collab 不可能可靠理解所有用户项目；文件选择应由掌握任务上下文的宿主负责                        | BEH-003、产品边界、VER-003                                             |
| send 与 wait 分离         | 用户在重做讨论中确认                               | 已确认 | Pro 可能长时间生成；提交不应占住宿主或阻止其他任务继续工作                                     | BEH-003、BEH-004、BEH-006、技术设计                                    |
| 原始回复交给宿主解释      | 用户在重做讨论中确认                               | 已确认 | 固定 response schema、diff 或 receipt 解析会把具体任务协议写死，并制造错误 blocker             | BEH-004、产品边界、VER-004                                             |
| close 与 Web archive 分离 | 用户在重做讨论中确认                               | 已确认 | 本地进程生命周期与 Web conversation 生命周期是两种独立副作用，必须由不同显式命令触发           | BEH-008、BEH-009、VER-008、VER-009                                     |
| Playwright CLI 浏览器边界 | 用户 2026-08-03 当前修订要求                       | 已确认 | 固定版本外部 CLI 已提供 session、profile 和页面命令；无需增加浏览器库封装或运行时 dependency   | 技术基线、Browser boundary、VER-001–VER-006、VER-008、VER-009、VER-012 |
| SQLite 与正文分离         | 用户 2026-08-03 当前修订要求                       | 已确认 | 结构化状态需要事务和跨进程查询；prompt/response 正文仍应保持为直接可读、逐 turn 不覆盖的文件   | State store、Artifact store、VER-007、VER-011、VER-012                 |
| 不兼容旧实现              | 用户要求从零推翻重做，且此前明确不做兼容           | 已确认 | 迁移旧 task、bundle 和数据库会把已经拒绝的复杂度带回新产品                                     | 产品边界、完成条件                                                     |

## 未决事项

| 未决项               | 当前状态与影响                                                                                                                                                              | 阻塞范围   | 解除权威 | 执行处理                                                                                          |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | -------- | ------------------------------------------------------------------------------------------------- |
| 独立任务认证副本机制 | 待验证使用完整 profile 副本、storage state 或其他机制；会改变 BEH-001、BEH-002、BEH-006 的 Playwright profile 建立方式和 VER-001、VER-002、VER-006 的执行，但不改变产品合同 | 实现、验证 | 外部事实 | 自主调查：在一次 setup 后真实启动两个独立 named session，并验证同时登录、profile 隔离和关闭清理   |
| ChatGPT Web 页面合同 | Playwright CLI 已固定，但 selector、完成检测、页面内 Copy response 拦截和 Archive 动作仍需新项目 live 证据；会改变 `browser.ts` 与 VER-003–VER-006、VER-009 的具体执行      | 实现、验证 | 外部事实 | 自主调查：使用固定 CLI 版本运行 live Pro conversation，验证上传、发送、长时间等待、并发捕获和归档 |

## 参考资料

| ID      | 名称                              | 位置                                                                  | 版本或日期                |
| ------- | --------------------------------- | --------------------------------------------------------------------- | ------------------------- |
| REF-001 | Playwright CLI README             | https://github.com/microsoft/playwright-cli/blob/main/README.md       | `main`，访问于 2026-08-03 |
