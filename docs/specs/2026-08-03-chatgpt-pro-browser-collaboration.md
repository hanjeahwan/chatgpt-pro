...
- **验收条件**：`start` 成功后、首次 `send` 之前不存在单独的启动 user turn；首轮 Web prompt 与指定文件一致，且完整包含一份固定协作合同和当前任务。输入选择层的文件访问记录只包含指定 prompt 和附件；未列出的仓库输入不会被该层枚举、打开、打包或上传；调用方在消息提交完成后取得 `turnId`。附件上传期间中断且尚未释放提交命令时，恢复后页面没有残留或重复附件，也没有挂起 file chooser，原 turn 记为 `failed`，只有宿主再次显式 `send` 才建立新 turn，且该 prompt 仍包含完整协作合同；通过关闭并重建实现的恢复不改变 task 身份绑定；提交结果不明确时不会产生自动重发，后续行为遵守 BEH-013。
- **状态**：已确认。
