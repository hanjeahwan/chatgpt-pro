...
- **执行或检查**：修改原 prompt 文件；检查 SQLite 与 turn 目录；对一个自动恢复、两个提交裁决分支和一个 response 失败裁决检查 operation evidence、resolution source 或 turn error；关闭任务后用新的 CLI 进程复读。
- **通过证据**：task、turn、user turn identity、artifact 顺序和路径可查询；prompt、response、返回文件保持原内容；发送附件正文没有副本；自动恢复与人工裁决可区分，人工记录包含结论、页面 identity、时间和最终状态；response 失败裁决还记录目标 user turn 与 Stop 处理结果。
