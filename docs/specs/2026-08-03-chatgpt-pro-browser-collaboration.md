...
- **最终捕获临界段**：recovered capture 的摘要通过后，必须在点击 Copy 的同一个无 await 页面同步临界段先确认 `location.hostname` 与 canonical `/c/<expectedConversationId>`，再确认唯一绑定 assistant、与摘要前快照逐字相同的 canonical 文本和唯一可见 Copy，并立即 click；任一项漂移都必须在 Copy 前失败。最终临界段不得因新页面具有相同 assistant DOM identity、文本或 Copy 而接受其他 conversation，也不得增加新的持久状态。
- **可观察行为**：Collab 在观察窗口内检查该 turn 的 Pro 回复。正常完成条件保持为目标 assistant 的 Copy 可见且页面没有可见的 `Stop answering`，完成协议同时返回经过检查的 assistant DOM identity、完成模式和目标内容 fingerprint。Fingerprint 必须是目标 assistant canonical DOM 文本按 UTF-8 编码计算的浏览器 SHA-256 小写 hex；observation 与 capture 使用同一 Web Crypto/TextEncoder 语义，算法或编码不可用时 fail closed 为页面合同错误。若同一目标 assistant 的 Copy 已可见、回复内容按页面单调时间稳定达到默认 30 秒 stall grace，但 `Stop answering` 持续可见，浏览器层对该本地 turn 与目标 DOM identity 至多执行一次页面 reload；内容变化立即重置 `stableSince`，该状态跨 bounded observation 调用保存在带本地 `turnId` 的 sessionStorage key 中。reload 前必须把该目标 assistant identity 与 canonical conversation 和本地 `turnId` 一起持久绑定；reload 后及后续 observation/capture 只接受该绑定目标，不能因窗口跨越、DOM 新增或 hydration 后 latest assistant 改变而漂移。reload 使用观察 deadline 的剩余 timeout；deadline 不足返回 `pending`，reload 拒绝或超时先回滚 attempt marker；reload 后重新校验 canonical conversation、目标 turn、Copy 与 Stop。若 Stop 消失，沿普通完成路径重新稳定检查；若 Stop 仍可见，则以 reload 后该目标当前内容作为新 baseline，内容每次变化重置 30 秒稳定窗口，目标内容连续稳定 30 秒后以 `recovered-stuck` 模式报告完成。Collab 将完成协议返回的 assistant identity、模式和 fingerprint 显式传给 capture；capture 同时核对 canonical conversation、该 identity 与 Copy，且 recovered 模式必须核对相同 SHA-256 fingerprint。摘要通过后，capture 必须在点击 Copy 的同一个页面同步临界段重新定位唯一绑定目标、逐字比较当前 canonical 文本与摘要前快照并重新确认唯一可见 Copy；最终比较与 click 之间不得 await 或跨 Playwright roundtrip。任何变化均不得点击 Copy 或进入 `capturing`，按 pending/页面合同错误返回。网页报告回复完成后，Collab 在本次 `captureTimeoutMs` 的宿主侧单调 deadline 内取得 Web 端完整 Copy response 与完整有序 artifact 描述，再以一个 SQLite 事务建立 `capturing` 边界；事务后把原始文字写入该 turn 专属且不覆盖其他 turn 的 `response.md`，并完成 BEH-012 的返回文件捕获。`observationWindowMs` 只约束网页完成观察；`captureTimeoutMs` 从网页报告完成或本次调用开始恢复 `capturing` turn 时计时，约束本次完整文字与剩余文件捕获。观察窗口到期而网页尚未报告完成时返回 `pending`；捕获超时或浏览器发生真实错误时返回错误。
- **验收条件**：reload 不点击 Stop、不因文字看似完整而直接捕获，也不产生宿主中间结果；reload 本身不建立完成事实。`completed` 只在 reload 后或正常路径上重新满足既有完成条件，且 `response.md` 与全部返回文件均已落盘后返回；`pending` 是已成功提交但尚未取得完整 Copy response 与 artifact 描述并建立原子 `capturing` 边界的本地持久化状态，不声称 Pro 必然仍在生成。取得完整数据前超时返回 `CAPTURE_TIMEOUT` 或发生其他真实错误时 turn 保持 `pending`，后续 `wait` 重新观察并重取 Copy；进入 `capturing` 后超时、中断或下载失败则保持 `capturing`，后续 `wait` 使用新的 `captureTimeoutMs` 按恢复矩阵继续。同一 completed turn 再次 `wait` 返回同一 `responsePath` 和文件路径；调用方中断不会停止远端生成，后续仍可再次 `wait`。
- **状态**：已确认。
...
- `wait` 以最新 user turn 为锚点，只接受其后的 assistant turn；该 assistant 的 Copy 按钮可见且页面没有可见的 `Stop answering` 按钮时才判定完成，并把该 assistant 的稳定 DOM identity、完成模式和内容 fingerprint 返回给 capture；fingerprint 为 canonical DOM 文本 UTF-8 的 Web Crypto SHA-256 小写 hex，计算失败即页面合同错误。若同一目标 assistant 的 Copy 可见、文字内容按页面单调时间稳定达到默认 30 秒 stall grace，但 Stop 持续可见，页面脚本以 conversation ID、本地 `turnId` 与目标 turn DOM identity 记录 reload 恢复和稳定状态，对该目标 turn 至多 reload 一次；内容变化立即重置 `stableSince`，reload 的 timeout 不超过观察 deadline 剩余预算，失败时回滚 attempt marker。reload 前持久化目标绑定，reload 后及后续 observation/capture 均只接受该绑定目标；目标暂不可见时保持 `pending`，不得回退到 latest assistant。reload 后 Stop 消失时重新满足普通稳定检查即可完成；Stop 仍可见时以 reload 后内容建立新的 fingerprint/stableSince baseline，内容连续稳定 30 秒可报告 recovered completion。target 或 conversation 改变时返回页面合同漂移；Copy/Stop 仍不满足正常条件时继续内部检查并按观察窗口返回 `pending`。capture 必须核对 observation 返回的 assistant identity，不能重新隐式接受 T2；recovered 模式还必须核对 fingerprint，变化时不得点击 Copy 或进入 capturing。该恢复不得点击 Stop，也不得把内容稳定本身当作完成。回复捕获在点击该 assistant 的 Copy 前临时拦截当前页面 `navigator.clipboard.write`/`writeText`，取得同一个 ClipboardItem 的 `text/plain` 和 `text/html` 后恢复原方法；`text/plain` 原样保存为 response，`text/html` 只用于当次文件发现。任一类型缺失时返回页面合同漂移，不读取或写入操作系统全局剪贴板。
...
- **执行或检查**：使用足够的观察窗口与捕获超时执行 `wait`；检查目标 assistant、`Stop answering` 与 Copy 状态，将页面内 Copy response 和 `response.md` 逐字节比较，再重复 wait。另复用一个回复内容与 Copy 已稳定、但 Stop/加载指示持续可见的真实目标 turn，只调用一次 `wait`，记录该目标 turn 的 reload 次数、reload 前后 canonical conversation/turn identity、返回的 assistant identity、completion mode/fingerprint 以及 reload 后的 Copy/Stop 检查；自动化 fixture 还覆盖 T1 observation 后 capture 前切换 T2、local turn key 隔离、30 秒 stall grace、post-reload 内容变化重置、慢 reload deadline、拒绝后 marker 回滚恢复，以及 recovered capture 摘要等待期间同一目标文本变化或导航到具有相同 turn identity/text/Copy 的其他 conversation。
- **通过证据**：正常路径返回 completed，内容一致，没有读取系统剪贴板，artifactPaths 为空，重复调用返回同一路径且旧 response 未覆盖；卡滞指示场景只 reload 一次，conversation 与目标 turn 不变，reload 后 Stop 消失沿普通路径完成，或 Stop 仍可见时目标内容连续稳定 30 秒并以 recovered mode/fingerprint 完成；capture 若 fingerprint 变化、摘要后同步终检发现文本变化或 canonical conversation 漂移则拒绝，完全恢复页面剪贴板拦截，不点击 Copy、不进入 capturing。自动化控制场景必须真实执行最终页面 callback，证明 6 个短样本不会提前 reload、reload 后 target 缺失/T2 不替代、post-reload 内容变化重置、reload deadline 有界且失败可重试，不会点击 Stop 或仅凭内容稳定捕获。修复后 live VER-004 尚未重跑，不能声明通过。
