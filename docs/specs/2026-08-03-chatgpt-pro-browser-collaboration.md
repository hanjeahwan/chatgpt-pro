...
- `wait` 以最新 user turn 为锚点，只接受其后的 assistant turn；该 assistant 的 Copy 按钮可见且页面没有可见的 `Stop answering` 按钮时才判定完成，并把该 assistant 的稳定 DOM identity 返回给 capture。若同一目标 assistant 的 Copy 可见、文字内容按页面单调时间稳定达到默认 30 秒 stall grace，但 Stop 持续可见，页面脚本以 conversation ID、本地 `turnId` 与目标 turn DOM identity 记录 reload 恢复和稳定状态，对该目标 turn 至多 reload 一次；内容变化立即重置 `stableSince`，reload 的 timeout 不超过观察 deadline 剩余预算，失败时回滚 attempt marker。reload 前持久化目标绑定，reload 后及后续 observation/capture 均只接受该绑定目标；目标暂不可见时保持 `pending`，不得回退到 latest assistant。reload 后必须重新确认 canonical conversation 未变、仍是同一目标 assistant、Copy 唯一可见且 Stop 不可见，并重新满足既有稳定检查次数，才报告完成。目标 turn 或 conversation 改变时返回页面合同漂移；Copy/Stop 仍不满足正常条件时继续内部检查并按观察窗口返回 `pending`。capture 必须核对 observation 返回的 assistant identity，不能重新隐式接受 T2；该恢复不得点击 Stop，也不得把内容稳定本身当作完成。回复捕获在点击该 assistant 的 Copy 前临时拦截当前页面 `navigator.clipboard.write`/`writeText`，取得同一个 ClipboardItem 的 `text/plain` 和 `text/html` 后恢复原方法；`text/plain` 原样保存为 response，`text/html` 只用于当次文件发现。任一类型缺失时返回页面合同漂移，不读取或写入操作系统全局剪贴板。
...
- **执行或检查**：使用足够的观察窗口与捕获超时执行 `wait`；检查目标 assistant、`Stop answering` 与 Copy 状态，将页面内 Copy response 和 `response.md` 逐字节比较，再重复 wait。另复用一个回复内容与 Copy 已稳定、但 Stop/加载指示持续可见的真实目标 turn，只调用一次 `wait`，记录该目标 turn 的 reload 次数、reload 前后 canonical conversation/turn identity、返回的 assistant identity 以及 reload 后的 Copy/Stop 正常完成检查；自动化 fixture 还覆盖 T1 observation 后 capture 前切换 T2、local turn key 隔离、30 秒 stall grace、慢 reload deadline 与拒绝后 marker 回滚恢复。
- **通过证据**：正常路径返回 completed，内容一致，没有读取系统剪贴板，artifactPaths 为空，重复调用返回同一路径且旧 response 未覆盖；卡滞指示场景只 reload 一次，conversation 与目标 turn 不变，且只在 reload 后 Copy 唯一可见、Stop 不可见并重新满足正常稳定条件后返回 completed。自动化控制场景证明 6 个短样本不会提前 reload、reload 后 Stop 仍可见时返回 `pending`、reload deadline 有界且失败可重试、capture 不接受替换后的 T2，不会点击 Stop 或仅凭内容稳定捕获。修复后 live VER-004 尚未重跑，不能声明通过。
