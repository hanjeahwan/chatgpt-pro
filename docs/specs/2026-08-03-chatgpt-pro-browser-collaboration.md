...
| `resolve-turn`       | active `taskId`、`turnId`、`failed`                                                 | 页面验证并在必要时显式停止目标 response，把 pending turn 置为 `failed`，返回 `nextAction: send`             | 非 pending turn；conversation 或 user turn 不符；有后续 user turn；无法验证安全 composer |
