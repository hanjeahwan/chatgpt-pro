...
- **可观察行为**：Collab 打开一次交互式登录流程，并在用户完成登录后保留可供后续任务加载的本地认证源；`setup` 完成后关闭本次设置使用的浏览器。`setup` 在认证源保存前后被中断时，后续同一命令先核对持久状态：认证源未保存时重新进入登录流程；认证源已经有效时只完成尚未完成的 setup session 清理，不要求用户重复登录。setup owner 退出时，其仍持有实际子进程的专用 command gate 有界终止 command；后续 `setup` 对 dead owner 记录的残留 gate 或 command 只做有界等待，进程退出后沿同一 journal 恢复，不按持久 PID 发送信号。前一 owner 仍存活时立即返回 busy。同一次 setup 已用隔离 session 证明现有 seed 已认证时，后续 seed 步骤复用该 operation evidence，不重复验证；只有页面明确显示可见 `Log in` 或 `Sign up` 才判定 seed 未认证，CLI、超时、协议或页面合同异常保留原错误。
- **验收条件**：完成一次人工登录后，两个后续任务都能在不再次要求用户登录的情况下进入已登录的 ChatGPT Web。认证源保存前中断不会留下成功记录，保存后但浏览器关闭前中断可由后续 `setup` 清理；owner 退出后，setup command 即使忽略 graceful signal 或 PID 尚未写入 lease，专用 gate 仍会有界终止自己实际持有的 command；后续 `setup` 不向持久 PID 发信号；恢复不删除有效认证源，也不把无法验证的认证源报告为成功。
- **状态**：已确认。
...
- **触发与前置条件**：宿主对活动任务执行 `send(taskId, promptPath, attachmentPaths[])`；该任务没有未完成 turn；所有传入路径均由宿主明确指定且当前可读。prompt 文件是有效 UTF-8，解码后的全文首尾均无空白，包括终止换行。新 task 尚未绑定 conversation 时，宿主已按 Skill 把固定协作合同与当前任务写入同一 prompt 文件。
- **可观察行为**：新 task 的第一条 user message 同时包含固定协作合同、当前任务和当轮附件；协作合同定义 Pro 作为承担有界任务的独立协作者，明确双方责任、宿主环境可观察边界、会话工具使用、前提质疑、假设与阻塞处理、后续轮次连续性和宿主最终验证与集成责任，并要求 Pro 直接处理当前任务而不只确认模式，但不要求固定回复格式。Collab 输入选择层只解析并读取 `promptPath` 与 `attachmentPaths` 中明确列出的文件，不隐式注入、trim 或改写 prompt；prompt 存在首尾空白时，在任何状态持久化或浏览器动作前返回 `PROMPT_NOT_VERBATIM_PROVABLE`。输入通过后，它先持久化本轮输入与提交阶段，再向该任务的 conversation 提交一次消息，随后立即返回唯一 `turnId`，不等待 Pro 完成回复。显式附件发送中，每个上传准备 run 返回有效 upload-ready envelope 即就绪；准备 run 结果为空时，因固定 CLI 的 modal-handoff 竞速只能作为暂定就绪——空输出单独从不证明成功，只有紧接着的 upload 命令成功才可证明就绪，非空非 envelope 输出或显式 CLI 工具错误是失败而非暂定就绪。若立即 upload 无法证明成功，不得发生提交；clean-or-rebuild composer 路径必须在失败 send 返回前证明安全 composer，安全恢复的 task 保持活动并允许一次后续显式 send。提交前中断时，恢复清理或重建为可安全发送的相同 composer；页面内清理被挂起的 file chooser（modal state）阻塞、无法证明安全时，恢复关闭同名 session 并从 seed 重建，已绑定 conversation 的任务恢复精确 canonical conversation，未绑定的任务恢复固定 Project 的空白 composer，重建后验证安全。提交命令已经释放但结果未返回时，Collab 先通过页面证据自动判断是否已提交，不能证明时将 turn 保持为 `unknown-submission`，禁止自动重发。
- **验收条件**：`start` 成功后、首次 `send` 之前不存在单独的启动 user turn；首轮 Web prompt 与指定文件一致，且完整包含一份固定协作合同和当前任务。输入选择层的文件访问记录只包含指定 prompt 和附件；未列出的仓库输入不会被该层枚举、打开、打包或上传；调用方在消息提交完成后取得 `turnId`。上传准备返回有效 envelope，或返回空结果但紧接着的 upload 成功时，继续提交；空准备结果单独不证明就绪，空结果后 upload 工具错误或非空非 envelope 输出时不提交消息，进入 clean-or-rebuild 并保留具体固定 CLI 错误，安全恢复后 task 保持活动且允许一次后续显式 `send`。附件上传期间中断且尚未释放提交命令时，恢复后页面没有残留或重复附件，也没有挂起 file chooser，原 turn 记为 `failed`，只有宿主再次显式 `send` 才建立新 turn，且该 prompt 仍包含完整协作合同；通过关闭并重建实现的恢复不改变 task 身份绑定；提交结果不明确时不会产生自动重发，后续行为遵守 BEH-013。
- **状态**：已确认。

### BEH-004 等待并捕获原始回复

- **触发与前置条件**：宿主执行 `wait(taskId, turnId, observationWindowMs, captureTimeoutMs)`，该 turn 已提交，且两个时长都是宿主提供的有限正整数。
- **可观察行为**：Collab 在观察窗口内检查该 turn 的 Pro 回复。只有目标 assistant 的 Copy 可见、页面没有可见的 `Stop answering`，并满足既有稳定检查时才报告网页完成；完成观察同时返回经过检查的 assistant DOM identity，本次 pending→capture 显式传入该 identity，capture 只接受同一个 assistant，不重新选择后来出现的 latest assistant。网页报告回复完成后，Collab 在本次 `captureTimeoutMs` 的宿主侧单调 deadline 内取得 Web 端完整 Copy response 与完整有序 artifact 描述，再以一个 SQLite 事务建立 `capturing` 边界；事务后把原始文字写入该 turn 专属且不覆盖其他 turn 的 `response.md`，并完成 BEH-012 的返回文件捕获。`observationWindowMs` 只约束网页完成观察；`captureTimeoutMs` 从网页报告完成或本次调用开始恢复 `capturing` turn 时计时，约束本次完整文字与剩余文件捕获。观察窗口到期而网页尚未报告完成时返回 `pending`；捕获超时或浏览器发生真实错误时返回错误。观察期间，Collab 以单调时钟从本次 `wait` 开始计时，未捕获持续每达到 `300000ms` 就无条件 reload 当前已绑定的 canonical conversation；reload 触发只依赖单调时间，不检查 Copy、`Stop answering`、Submit、文本或其他 DOM 完成/卡死信号，也不因内容稳定或看似完整而捕获。reload 耗时计入原 `observationWindowMs`，只在剩余预算内继续；每次 reload 后核对页面仍绑定数据库保存的同一 exact canonical conversation（`origin + pathname`、无 query/fragment，与保存值完全一致）与目标 user turn，再走既有 observation 与 capture 路径，未捕获期间可按同一周期重复，观察窗口到期仍正常返回 `pending`。Collab 不点击 Stop、不改变 turn 状态、不自动失败、不发送 continuation。只有用户明确确认目标 Pro response 已失败或终止后，宿主才可按 BEH-013 显式裁决该 pending turn；用户明确放弃永久无法完成的 capturing turn 时，同一命令只做本地裁决并保留冻结结果。
- **验收条件**：`completed` 只在正常完成条件成立，且 `response.md` 与全部返回文件均已落盘后返回。观察与捕获之间若原 assistant identity 缺失、重复或漂移，必须在 Copy 前失败，turn 保持 `pending`；后续 `wait` 重新观察。`pending` 是已成功提交但尚未取得完整 Copy response 与 artifact 描述并建立原子 `capturing` 边界的本地持久化状态，不声称 Pro 必然仍在生成，也不因 repeated pending、超时、reload 或内容稳定自动变为 failed。同一次 `wait` 内未捕获持续每达到 `300000ms` 时，Collab 无条件 reload 当前已绑定 canonical conversation；触发只依赖单调时间，不检查 Copy、`Stop answering`、Submit、文本或其他 DOM 完成/卡死信号。reload 耗时计入原 `observationWindowMs`，只在剩余预算内继续，窗口到期仍正常返回 `pending`，未捕获期间可按同一周期重复；每次 reload 后核对数据库保存的同一 exact canonical conversation URL（`origin + pathname`、无 query/fragment）与 canonical identity、目标 user turn，核对失败时保持 `pending` 且不捕获、不自动失败，核对成功后继续既有观察与捕获。reload 不点击 Stop、不改变 turn 持久状态、不发送 continuation。取得完整数据前超时返回 `CAPTURE_TIMEOUT` 或发生其他真实错误时 turn 保持 `pending`，后续 `wait` 重新观察并重取 Copy；进入 `capturing` 后超时、中断或下载失败则保持 `capturing`，后续 `wait` 使用新的 `captureTimeoutMs` 按恢复矩阵继续，且不新增持久 assistant identity；只有用户显式裁决时才可本地转为 `failed`，冻结的 response 路径、已发布 `response.md`、全部 artifact 行、完成状态与 pending 错误保持不变。同一 completed turn 再次 `wait` 返回同一 `responsePath` 和文件路径；调用方中断不会停止远端生成，后续仍可再次 `wait`。
- **状态**：已确认。

### BEH-005 同一任务持续多轮

- **触发与前置条件**：前一 turn 已完成，或其 Pro response 已由用户按 BEH-013 明确裁决为 failed；任务仍活动。
- **可观察行为**：宿主可再次执行 BEH-003 和 BEH-004；所有 turn 继续使用同一个浏览器进程和同一个 conversation，并分别保存输入与回复。后续 prompt 依赖首轮已建立的协作上下文，不重复固定协作合同。failed response 后的 continuation 是宿主另行选择并显式发送的新 turn，不由裁决命令自动生成。
- **验收条件**：后续回复能使用此前 conversation 的上下文；每个 turn 有独立 `turnId` 和文件记录；同一任务同一时间最多存在一个未完成 turn。failed response 裁决与 continuation 之间没有 Collab 自动创建的 user turn。
- **状态**：已确认。
...
- **可观察行为**：Collab 在 `~/.local/chatgpt-pro-collab/` 下保留 task、turn、未完成浏览器副作用与返回文件元数据，并为每个 turn 保存 prompt 副本、原始附件路径清单、状态、完整 response 和 BEH-012 下载的文件。未完成浏览器副作用恢复采用的页面证据、自动判定或人工提交裁决来源进入对应 operation 或 turn 记录；普通 closed task 或缺失 browser 的 session 重建只更新既有 task 与 session 状态，不新增 recovery operation。新 turn 不覆盖旧 turn；宿主之后改写原始 `promptPath` 不改变已保存副本。
- **验收条件**：任务关闭后仍能按 `taskId` 和 `turnId` 复原文字交互次序、返回文件顺序、本地路径，以及未完成浏览器副作用的恢复与裁决结果；重复使用同一输入或返回文件名不会改变旧记录；发送附件正文不因审计目的被额外复制。人工提交裁决与 pending response 裁决记录用户结论、实际验证的页面身份、时间和结果；capturing 裁决不读取页面，只记录用户结论、时间、来源状态和结果；均不把人工结论伪装为自动证明。
- **状态**：已确认。

### BEH-008 关闭并恢复本地任务

- **触发与前置条件**：宿主执行 `close(taskId)` 释放暂时不需要的 task browser，或对 `closed` task 执行 `recover(taskId)` 继续协作。
- **可观察行为**：Collab 在终止浏览器前把 task 持久化为 `closing`。每次启动或重建 task browser 时持久化固定 CLI 返回的 daemon PID；`close` 先关闭同名 session，daemon 未退出时仅在进程组仍能证明属于该 task session 的条件下定向终止，并确认 session 与 daemon 均不存在后再置为可恢复暂停态 `closed`，同时保留 BEH-007 的 transcript、conversation binding 和 BEH-001 的共享认证源。`status` 对 `closed` task 返回 `nextAction: recover`；`recover` 从共享 seed 重建同名 session，已绑定 task 只打开并验证记录的 canonical conversation，未绑定 task 只恢复固定 Project 的空白 composer，验证成功后才把 task 重新置为 `active`。`close` 不归档或删除 Web conversation，也不修改用户仓库。
- **验收条件**：关闭后浏览器进程不再存在，transcript、conversation binding 和共享认证源仍可读取，Web conversation 保持原状态；`close` 中断后再次执行可继续清理，重复 `close` 不产生额外副作用。closed task 恢复后继续使用同一 `taskId`、named session 和 canonical conversation；completed turn 后可显式发送 feedback，pending 或 capturing turn 可继续原 `wait`，均不创建或迁移 conversation、不自动发送消息。恢复失败时 task 保持 `closed`，允许以后再次显式 `recover`。
- **状态**：已确认。
...
- **触发与前置条件**：宿主执行 `status(taskId)` 检查任务，执行 `recover(taskId)` 恢复中断操作，对无法自动判定的 `unknown-submission` 执行 `resolve-submission`，或在用户明确确认已提交的 Pro response 失败、终止或永久无法完成捕获后执行 `resolve-turn <taskId> <turnId> failed`；本地 task、turn 或 operation 记录仍可读取。
- **可观察行为**：`status` 只读取持久状态与当前 browser session 可用性，返回 task、turn、未完成 operation、最近证据、错误和唯一安全的 `nextAction`，不执行远端副作用。`recover` 取得该 task 的互斥 lease 后，按持久阶段与页面后置条件继续 setup 之外的启动、发送准备、浏览器重建、closed task 重新激活或归档恢复；`closing` task 的下一步仍是幂等 `close`。active task 的 browser 缺失或 task 已为 `closed` 时，`status` 先返回 `recover`；恢复成功后，`pending` 或 `capturing` turn 的下一步才是使用原参数合同执行 `wait`。浏览器进程退出时，未绑定 conversation 的任务从共享认证源重建同一 Project composer；已经绑定的任务只导航到记录的 canonical URL，并核对唯一 conversation identity 与相关 turn 后恢复。页面内清理无法证明安全（如残留挂起 file chooser 的 modal state）时，`recover` 关闭同名 session 并从 seed 按同一身份约束重建：已绑定任务恢复精确 canonical conversation，未绑定任务恢复固定 Project 空白 composer，随后验证安全。任何恢复都不自动重发消息。`resolve-turn ... failed` 使用用户提供的终端失败事实并取得同一 task lease：pending 分支保留现有浏览器重建、唯一目标 user turn、无后续 user turn、安全 composer 与至多一次 Stop 验证；capturing 分支不读取或操作页面，只在一个本地事务中保留冻结结果并把 turn 置为 failed。裁决后只读探测当前 browser session，并按最新持久状态返回唯一安全的 `nextAction`，不代替宿主发送 continuation。
- **验收条件**：可自动证明的状态由系统恢复到既有安全命令入口；closed task 只在同一 task、session 与 conversation identity 验证成功后变为 `active`，失败时保持 `closed`。active task 的 browser 缺失以及 closed task 均先返回 `nextAction: recover`，恢复后按未完成 turn 返回 `wait` 或在没有待继续状态时返回 `none`。无法自动证明首次消息是否提交且尚无 canonical identity 时返回 `nextAction: resolve-submission`。`submitted` 裁决必须提供 canonical conversation URL，Collab 必须在已登录页面中验证其属于唯一 `chatgpt-pro-collab` Project；task 已有绑定时还必须与已记录 conversation identity 相同，随后把位于已记录前一 turn 锚点之后、与已保存 prompt 及有序附件名称一致的唯一 user turn 绑定到原 `turnId` 后才能置为 `pending`。`not-submitted` 裁决由用户提供“未提交”的权限性事实，Collab 仍须验证页面已经恢复为该 task 已绑定 conversation 的安全 composer，或在尚未绑定时恢复为唯一目标 Project 的空白安全 composer，且预期锚点之后没有匹配 user turn 或进行中的发送状态；之后把原 turn 置为 `failed`，由宿主另行显式 `send`。已提交的人工作为同一 operation 的 durable outcome：输出丢失后，同 verdict 且规范化后的 submitted URL 相同的重复调用只返回当前 status，不重复页面验证、提交或 Stop；不同 verdict 或规范化后的 submitted URL 不同则返回冲突并保持现状。`resolve-turn ... failed` 只接受 active task 的 pending 或 capturing turn；pending 的 conversation 或 user-turn identity 不符、存在后续 user turn、Stop 无法结束、composer 不安全或页面不可用时保持原状态；capturing 裁决不读取页面，不写入新的页面证明，并保留 responsePath、已发布 response、completed artifact、pending artifact 及其错误。成功时 turn error 记录来源状态；pending 另记录页面身份与 Stop 结果。重复调用不产生额外页面动作或 user turn，但重新计算当前 status：已有新 pending 或 capturing turn 时返回 `wait`，session 缺失时返回 `recover`，探测失败时返回 `unknown` 与诊断且不返回 `send`；仅在 active task 无未完成 turn、无未提交 operation 且 session 可用时返回 `send`。后续 continuation 只能由宿主显式 `send`。completed、sending、unknown-submission 或其他 failed turn 仍拒绝；所有裁决都不自动发送消息。
- **状态**：已确认。
...
- 当前 `wait` 已支持有限观察、原子冻结 response 与 artifact 集、复用已完成文件及继续未完成下载；`close` 可重复调用且先持久化 `closing` 状态再终止浏览器。当前 `start` 接受调用方提供的稳定 `taskId`，中断后可用同一身份继续恢复；`archive` 已覆盖点击前后进程退出的持久后置条件核对；active 或 closed task 的 browser 退出后从 seed 与 canonical identity 重建，closed task 仅在身份验证成功后重新激活。
...
- 固定 CLI 的 `upload` 命令强制 `file` 参数，无法表达 MCP `browser_upload` 以空 `paths` 取消挂起 file chooser 的语义；`run-code` 与 `reload` 在 modal state（挂起 file chooser）下被 modal guard 拒绝，`close` 是 session 命令、不经过该 guard。draft 清理在页面内无法证明安全时，恢复按“浏览器副作用恢复”的 clean-or-rebuild 合同关闭同名 session 并从 seed 重建（BEH-003、BEH-013）。
- 固定 CLI 的 `run-code` 把页面函数包在 tab 的 `waitForCompletion` 中，该等待与 modal state 竞速：点击 `Add photos & files` 在页面函数解析完成前发出 fileChooser modal 时，`run-code` 提前返回且不添加本次结果，`--raw` 输出为空 stdout；该挂起 fileChooser 由下一个 `upload` 命令消费并清除（已按固定安装版本核验）。因此上传准备 run 的空结果只是 modal-handoff 的暂定就绪，必须由紧接着的 `upload` 成功证明，空结果单独既不是成功也不是失败（BEH-003）。
- `run-code` 的显式工具错误是纯文本结果，现有协议失败格式化只保留 `### Error` 标记。上传准备或清理失败返回宿主错误时，必须保留具体固定 CLI 错误文本，不得折叠为通用 protocol-envelope 缺失（BEH-003）。
...
- `wait` 以当前 turn 持久化的精确 user DOM identity 为锚点，只接受其后的 assistant turn；user target 缺失或不唯一时返回页面合同漂移，不退回 latest user turn。该 assistant 的 Copy 按钮可见、页面没有可见的 `Stop answering` 按钮且连续稳定检查通过时才判定完成。observation 返回该 assistant 的稳定 DOM identity，本次 pending→capture 显式传入并只接受该 exact identity；若 target 缺失、重复或漂移则在 Copy 前返回页面合同漂移，不重新隐式接受 T2。未捕获持续每达到 `300000ms` 时，`wait` 通过固定 CLI 的 `reload` 无条件重载当前已绑定 canonical conversation；触发只依赖单调时间，不检查 Stop、Copy、Submit、文本或其他 DOM 完成/卡死信号。reload 耗时计入原 `observationWindowMs`，只在剩余预算内继续；每次 reload 后重新核对数据库保存的 exact canonical conversation URL（`origin + pathname`、无 query/fragment）与 canonical identity、目标 user turn 再走既有完成检测，未捕获期间可按同一周期重复，观察窗口到期按观察 deadline 返回 `pending`。`wait` 不点击 Stop、不自动失败、不发送 continuation，也不按内容稳定性建立另一种完成模式。回复捕获在点击该 assistant 的 Copy 前临时拦截当前页面 `navigator.clipboard.write`/`writeText`，取得同一个 ClipboardItem 的 `text/plain` 和 `text/html` 后恢复原方法；`text/plain` 原样保存为 response，`text/html` 只用于当次文件发现。任一类型缺失时返回页面合同漂移，不读取或写入操作系统全局剪贴板。
...
| `send`               | `taskId`、有效 UTF-8 且首尾无空白的 `promptPath`、零或多个 `attachmentPath`         | 消息已提交并返回 `turnId`                                                                                   | task 非活动；本 task 已有未完成 turn；输入不可读或无法逐字证明；上传失败；提交结果不明  |
...
| `recover`            | `taskId`                                                                            | 页面内清理或关闭重建并验证安全后，恢复到可安全继续的既有命令入口；closed task 重新激活，并返回 `nextAction` | 清理与重建后页面后置条件仍无法取得；conversation 无权访问或身份不符；需要人工提交裁决   |
...
| `resolve-turn`       | active `taskId`、pending 或 capturing `turnId`、`failed`                            | pending 经页面验证；capturing 仅本地保留冻结结果；置为 `failed` 并返回当前安全 `nextAction`                 | 其他 turn 状态；pending 页面身份、后续 user turn、Stop 或 composer 无法验证             |
| `close`              | `taskId`                                                                            | 本地 browser 已关闭，task 进入可恢复暂停态                                                                  | 清理未完成时返回具体残留，不声称完整成功                                                |
| `archive`            | 活动 `taskId`                                                                       | 指定 Web conversation 已归档，task 页面已恢复原绑定                                                         | task 非活动；conversation 尚未建立；Web 后置条件无法判断                                |

`start` 的命令形式为 `start <taskId>`，`taskId` 必须是 canonical lowercase UUID v4；CLI 在打开数据库、创建目录或调用浏览器前拒绝其他格式。`wait` 的命令形式为 `wait <taskId> <turnId> <observationWindowMs> <captureTimeoutMs>`；提交人工裁决只允许 `resolve-submission <taskId> <turnId> submitted <conversationUrl>` 或 `resolve-submission <taskId> <turnId> not-submitted`，response 终端失败裁决只允许 `resolve-turn <taskId> <turnId> failed`。`conversationUrl` 必须由 URL parser 验证为无用户名、密码、query 和 fragment、无非默认端口的 `https://chatgpt.com/c/<conversationId>` 或 `https://chatgpt.com/g/g-p-<project>/c/<conversationId>` canonical URL，其他 origin 或路径在浏览器动作前拒绝。`status` 的 `nextAction` 只允许 `setup | start | send | wait | recover | resolve-submission | close | none`；用户提供的 response 失败事实不是持久状态推导结果，因此 `status` 不自动返回 `resolve-turn`。`pending` 是本次有限观察正常到期的成功结果，不是 turn 失败；捕获超时使用错误码 `CAPTURE_TIMEOUT`。完成结果和 pending 结果都包含 `taskId` 与 `turnId`。CLI 在一次调用中不得输出进度 JSON，错误只通过现有非零退出码与单条错误 JSON 返回。
...
每个 task 只定义 `starting`、`active`、`closing`、`closed`、`failed` 五种状态；每个 turn 定义 `sending`、`pending`、`capturing`、`completed`、`failed` 和 `unknown-submission`。`starting` 表示调用方身份与启动意图已经持久化，但目标 composer 尚未完成后置条件确认；`closing` 表示关闭意图已经持久化，后续只能继续 `close`，不能重建 browser；`closed` 表示 named session 已停止的可恢复暂停态，后续只能通过 `recover` 验证并重新激活原 task，不通过 `start` 新建身份。`pending` 表示提交已成功，但尚未取得完整 Copy response 与有序 artifact 描述并原子建立 `capturing` 边界；它是本地持久化阶段，不等同于 Pro 必然仍在生成。pending turn 只有在完整捕获后进入 `capturing`，或在用户明确提供 response 已失败/终止的事实且 `resolve-turn ... failed` 页面验证成功后进入 `failed`；时间、重复观察与周期 reload 本身不触发后一转换。`capturing` 表示原始 response 路径与完整 artifact 集已经冻结，后续只允许发布或核对原始文字并下载返回文件，不能再返回成功的 `pending` 结果；用户显式放弃永久无法完成的捕获时，`resolve-turn ... failed` 只做本地事务并保留冻结结果。`unknown-submission` 只用于浏览器在提交边界失败、实现无法证明消息是否已经发送的真实歧义，不能被自动重试掩盖。
...
SQLite 只保存协调状态和文件索引，不保存 prompt、response 或返回文件正文。目标 schema 包含六张表：
...
| `browser_process` | `task_id`                   | 唯一正整数 `pid`；关联该 task 最近一次启动或重建的固定 CLI daemon                                                                                                              |
...
`evidence_json` 在没有实际观察时编码为 JSON `null`；非 `null` 时必须是只记录实际观察、不把推断记为事实并满足下列公共字段与各 kind 扩展字段的对象。公共字段为 `observedAt`、`sessionName`、可空 `pageUrl` 和 `postcondition`。setup evidence 另含 `seedValidated` 与 `sessionClosed`；start evidence 另含 Project identity、模型与 Power；send evidence 另含可空 conversation identity、可空 user turn identity、prompt 是否逐字匹配和有序附件名称匹配结果；archive evidence 另含 conversation identity、`archived` 与 `bindingRestored`。人工提交裁决另记录 `decision`、用户提供的 canonical URL 和页面验证结果；response 失败裁决不新增 operation evidence，pending 分支在 turn error 中记录用户结论、canonical 页面、目标 user turn 与 Stop 处理结果，capturing 分支只记录用户结论、`resolvedAt` 与 `sourceStatus`。不在 evidence 或 error 中复制 prompt 正文、附件字节、cookie 或 storage state。
...
`send` 先在一个事务中创建 `sending` turn 与 `send: draft: prepared` operation，再无覆盖地发布 prompt 副本；prompt 缺失或不一致时不得执行浏览器动作，恢复把原 turn 置为 `failed`。turn 在证明提交完成前保持 `sending`。每个附件在同一次页面操作中写入隐藏 file input，并只在 composer 已显示累计有序附件名称后完成该步。提交 command PID 放行前，operation 必须进入 `send: submit: effect-unknown`；页面观察到新 user turn 后还必须按前一 turn 锚点重新验证保存的 prompt 与有序附件名称，匹配唯一时才由单个事务把 turn 置为 `pending`，记录唯一 user DOM identity，绑定 canonical conversation identity，并把 operation 置为 `committed`。页面证据不匹配或不唯一时进入 `needs-decision`，不得把附件名称匹配记为已证明。经过 BEH-013 的 `submitted` 裁决后同样按该证据提交；`not-submitted` 裁决把 turn 置为 `failed` 后提交 operation。观察到网页完成后，turn 在取得 Copy response 与完整有序 artifact 描述期间仍保持 `pending`；超时、中断或浏览器错误不得留下 response 路径或部分 artifact 行。`resolve-turn ... failed` 在页面身份和安全 continuation composer 验证成功后，以一个事务把目标 turn 从 `pending` 置为 `failed` 并记录 human adjudication、canonical page、目标 user turn 与 Stop 处理结果；它不新增 operation kind、不写 response path、不发送消息。capturing 分支不读取页面，以一个本地事务只把目标 turn 从 `capturing` 置为 `failed` 并记录用户结论、时间与来源状态；既有 `response_path`、已发布 `response.md`、全部 artifact 行及其状态与错误保持不变。取得全部数据后，单个 `BEGIN IMMEDIATE` 事务必须同时把 turn 从 `pending` 置为 `capturing`、记录确定的 `response_path`，并按页面顺序建立全部 artifact 行；事务只能全部提交或全部回滚，不得把 `beginCapture` 与 artifact 建行拆成两个事务。事务提交后才发布 `response.md` 和返回文件。每个文件先写同目录临时文件，再无覆盖地发布到最终路径并标为 completed。只有 response 和全部 artifact 文件均可读时，turn 才能进入 `completed`。
...
| send operation 的 step 为 `draft`                 | 先 reload 并核对目标 composer 不含已选附件名称或已填充 file input；页面内无法证明安全时，关闭同名 named session 并从 seed 重建：已绑定 conversation 的任务恢复精确 canonical conversation，未绑定的任务恢复固定 Project 空白 composer；重建并验证安全后把原 turn 置为 `failed` 并提交 operation，返回 `nextAction: send` | 不继续上传，不提交消息，不保留残留或重复附件；新的消息必须由宿主显式再次 `send`；重建不迁移或猜测 conversation |
...
`status` 不取得 mutating operation lease，也不执行本表动作；它对同一数据库快照返回 `taskId`、`taskStatus`、可空 `turnId`、可空 `turnStatus`、`browserStatus: available | missing | unknown`、可空 `operationKind`、可空 `operationStep`、可空 `operationPhase`、可空 `operationProgress`、最近 evidence、最近 error 和唯一 `nextAction`。availability probe 受宿主 deadline 约束；CLI 失败或输出不可解析时，`status` 返回 `browserStatus: unknown` 并在 `error` 保留真实诊断，mutating 命令则直接失败，不能把 `unknown` 当成 `missing` 重建 session。`nextAction` 表示继续或解除当前持久工作流的系统选定动作，不枚举依赖用户新事实的 `resolve-turn` 等主动命令；没有待继续或待解除状态时为 `none`。`closing` 优先返回 `close`；closed task 或 active task 的 browser 确认缺失时优先返回 `recover`；恢复成功后再按 turn 返回 `wait` 或 `none`。`recover` 取得 lease 后只执行表中与当前持久状态匹配的一个恢复路径，并返回恢复后的同一组状态字段；若下一步是 `wait`、`close` 或人工提交裁决，只返回该 `nextAction`，不代替对应命令；probe 失败时本次调用明确失败，由下一次显式调用重试。

`start` 或 `recover` 每次新建 task browser 后立即把固定 CLI 返回的 daemon PID 关联到该 task；复用既有 session 不伪造 PID。`close` 在关闭浏览器前以事务把任一非 `closed` task 置为 `closing`；`closing` task 的 `status.nextAction` 固定为 `close`，`recover` 不得重建其 browser。后续 `close` 先关闭记录的 named session；若持久化 daemon 仍存在，只在当前进程组命令能证明属于该 session 时对该进程组发送 `SIGTERM`，宽限后仍存在则发送 `SIGKILL`。身份无法证明或终止失败时保留 PID 与 `closing` 状态并返回失败，不得使用全局 `kill-all`。定向清理成功后清除 PID，并在确认 session 不存在后把 task 置为 `closed`；状态写入失败时保持 `closing`，不得删除 transcript 或改回先前状态。`closed` 是可恢复暂停态：`recover` 重建并验证同一 task 身份后以事务改回 `active` 并清除当前 `closed_at`；重建或验证失败时保持 `closed`。同一 task 可重复执行 close/recover 周期。
...
- **执行或检查**：宿主先把 Skill 中的固定协作合同与当前任务写入同一 prompt 文件，再从工作区内外各选择明确的普通附件执行首次 `send`，记录输入选择层的路径解析、文件访问和读取调用；另以 live Web 检查 user turn 文字与实际上传项，不传入其他文件。确定性可执行夹具覆盖隐藏 file input 注入、每一步累计有序附件名称的可见后置条件、第二个附件失败、畸形或不匹配 envelope、页面清理成功和页面清理失败。分别在每个附件完成可见后置条件后且提交 command 放行前终止进程，再执行 `recover` 并重新进入显式 send 流程；页面内清理失败时实机复验关闭同名 session 并从 seed 重建的恢复（已绑定与未绑定任务各一）。正常提交另以保存的 prompt 和有序附件名称对新 user turn 做页面复核；零个或多个匹配均进入提交裁决，不直接记为成功。
- **通过证据**：首个 user turn 与指定 prompt 文件逐字一致，在一条消息内完整包含一份固定协作合同和当前任务，且没有独立启动 user turn。Pro 的第一个回复直接处理当前任务，或在真正阻塞时提出一个聚焦问题，不只确认协作模式。输入选择层只解析和访问指定 prompt 与附件；显式外部路径可上传；每次页面注入只在 composer 已显示全部累计有序附件名称后返回就绪；Web 没有未指定上传项或归档；正常路径只有在唯一 user turn 的 prompt 与附件名称再次匹配后返回 turnId。提交放行前的每个中断点都恢复为没有残留或重复附件的安全 composer，原 turn 标为 `failed`，未产生 user turn，也未自动发送；新的消息只有在宿主再次显式 `send` 后出现，且其 prompt 仍包含完整协作合同。上传准备或可见后置条件失败时不提交并调用 clean-or-rebuild；非空非 envelope 或畸形不匹配 envelope 保持合同漂移；cleanup 成功、cleanup 失败与页面脚本错误可区分；一次真实显式多附件发送恰好产生一个 user turn，附件按选择顺序全部到达。
- **证明边界**：不约束 Node.js、npm/npx、浏览器或操作系统读取其运行依赖、配置与缓存；不证明归档输入或 Pro 回复正确；附件可见后置条件以当前 ChatGPT composer DOM 与固定 CLI 版本行为为界，不做跨版本推断。
- **必需性**：必需。
...
- **执行或检查**：使用足够的观察窗口与捕获超时执行 `wait`；检查持久化 user identity、目标 assistant、`Stop answering` 与 Copy 状态，将页面内 Copy response 和 `response.md` 逐字节比较，再重复 wait。自动化 fixture 另覆盖目标 user 之后出现额外 user turn、observation 已判定 T1 完成但 capture 前页面新增 T2 的竞态、Stop 持续可见的观察到期路径，以及周期 reload：确定性模拟 stale 完成状态（服务端已有 response、页面未呈现可捕获完成控件）下，未捕获持续达到 `300000ms` 按单调时间触发 reload、reload 后 canonical conversation 与目标 user turn 核对成功与核对失败、reload 耗时耗尽 `observationWindowMs` 剩余预算后正常返回 pending、同一次 wait 内多次 reload 周期；正常捕获 fixture 必须真实执行页面内 clipboard install→Copy click/write→captured read→restore。真实 Web live 验证：让一个长生成 turn 跨过 `300000ms` 自动 reload 点，证明 reload 不终止远端生成、同一 canonical conversation 与目标 user turn 身份不变、最终完成捕获且无额外 user turn；自然偶发 stale 的再次实机复现列为证明边界，不作为每次实现必须可靠制造的前置。
- **通过证据**：正常路径返回 completed，内容一致，没有读取系统剪贴板，artifactPaths 为空，重复调用返回同一路径且旧 response 未覆盖；wait 只使用持久化 user identity，target 缺失或不唯一时不退回 latest user。observation 返回的 T1 identity 被显式传给 capture，页面新增 T2 时 capture 仍只接受 T1，T1 缺失或不唯一时不点击 Copy、不进入 `capturing`；Stop 持续可见时不点击 Stop、不自动失败、不发送 continuation，并在观察窗口到期后返回 pending。周期 reload 只由未捕获持续达到 `300000ms` 的单调时间触发，与 Copy、Stop、Submit、文本或其他 DOM 完成/卡死信号无关；reload 后身份核对成功时继续既有观察与捕获，核对失败时保持 `pending` 且不捕获、不自动失败；reload 耗时计入原 `observationWindowMs`，剩余预算耗尽时窗口到期正常返回 pending，同一次 wait 内多次 reload 周期可重复且 turn 持久状态不变。真实 Web 长生成 turn 跨过自动 reload 点后远端生成未被终止，同一 canonical conversation 与目标 user turn 身份不变，最终 completed 且无额外 user turn。页面内 clipboard fixture 逐字验证 `text/plain`、`text/html` 和失败/成功路径的全局恢复。
- **证明边界**：不覆盖返回文件下载、其他未知 ChatGPT Web 加载指示漂移或用户主动刷新；自然偶发 stale 状态的实机复现列为证明边界，不作为每次实现必须可靠制造的前置。
- **必需性**：必需。

### VER-005 同一任务多轮

- **覆盖对象**：BEH-005。
- **前置条件**：同一 task 的首个 turn 已完成；另有一个已由用户明确确认 response 失败或终止的 pending turn。
- **执行或检查**：先发送不重复固定协作合同、但引用上文的第二个 prompt 并等待；再对失败 response 执行 `resolve-turn ... failed`，由宿主另行显式发送针对中断点的 continuation prompt 并等待。
- **通过证据**：所有 turn 的 conversation 标识不变；第二个 user turn 与 continuation 均不包含固定协作合同，回复仍使用此前上下文；裁决本身不生成 user turn，continuation 只在显式 `send` 后出现；各 turn 文件独立。
...
- **执行或检查**：修改原 prompt 文件；检查 SQLite 与 turn 目录；对一个未完成浏览器副作用的自动恢复、两个提交裁决分支、一个 pending response 裁决和一个 capturing 本地裁决检查 operation evidence、resolution source 或 turn error；另执行普通 closed task 与缺失 browser 的 session 重建，关闭任务后用新的 CLI 进程复读。
- **通过证据**：task、turn、user turn identity、artifact 顺序和路径可查询；prompt、response、返回文件保持原内容；发送附件正文没有副本；未完成浏览器副作用的自动恢复与人工裁决可区分，普通 session 重建不新增 recovery operation；提交裁决与 pending response 裁决记录结论、实际页面 identity、时间和最终状态，pending response 裁决另记录目标 user turn 与 Stop 处理结果；capturing 裁决只记录结论、时间、来源状态和最终状态，不新增页面证明，已发布 response 与完整 artifact 状态保持不变。
...
- **执行或检查**：在浏览器关闭前、浏览器关闭后但 task 状态提交前分别终止 `close`，先执行 `status` 与 `recover`，再并发执行两次 `close`，检查持久化 daemon PID、对应进程组、Playwright session、本地目录与 Web；另用忽略 `SIGTERM` 的 task-owned daemon 夹具验证宽限后的定向 `SIGKILL`，并验证不匹配 session 的 PID 不被终止。真实 Web 只验证 completed turn 的 `close → status → recover` 连续场景：恢复后显式发送一条依赖首轮上下文的 feedback 并 wait，核对 taskId、named session 与 canonical conversation identity 不变。pending/capturing turn、未绑定 closed task、重建或 conversation identity 验证失败后再次 recover 与并发 recover 等状态和故障边界，由确定性 State/Browser 夹具证明。
- **通过证据**：关闭中断后 task 保持 `closing`，`status.nextAction` 为 `close`，`recover` 不重建 session；正常 session close 未带走 daemon 时，只终止持久化 PID 对应且可证明属于该 task session 的进程组，其他 session 不受影响；无法证明身份或终止失败时 PID 保留且 task 不进入 `closed`。关闭完成后只有同一个 `closed` task，named session、浏览器进程与 daemon 均消失，`status.nextAction` 为 `recover`；SQLite 中 task 对应 PID 已清除，transcript、返回文件与共享 seed 保留，Web 未归档。真实 Web 恢复只建立同名 session、打开同一 canonical conversation，并在身份验证成功后把 task 置为 `active`；completed turn 后的 feedback 成为同一 conversation 的新 turn，两轮 taskId、named session 与 canonical conversation identity 均不变。pending/capturing turn 恢复后返回 `wait` 并完成原 turn、未绑定恢复、验证失败时保持 closed，以及重复 close/recover 与并发 recover 无额外 session、conversation 或消息副作用，由确定性 State/Browser 夹具证明。
...
- **通过证据**：每条命令退出码为 0；入口不依赖宿主 package manifest；help 含稳定 taskId、两个 wait 时长参数、`status`、`recover`、两个 `resolve-submission` 形式与唯一 `resolve-turn <taskId> <turnId> failed` 形式；测试输出可追溯对应 `BEH-*`。
...
- **执行或检查**：先证明 `status` 对活动、closed、browser 缺失、operation 未完成和 `needs-decision` 状态只读。随后分别终止尚未首次发送的 task browser、已经绑定 conversation 且 turn 为 pending 的 task browser，以及提交 command 已放行但首次 conversation identity 尚未持久化的 browser；对前两者执行并发 `recover`，对第三者检查阻塞。另关闭一个已完成首轮的 task，按 `status.nextAction` 恢复并发送依赖首轮上下文的 feedback（真实 Web 验证）；带 pending turn 的 closed task 恢复后返回 `wait` 并继续原 wait，由确定性 State/Browser 夹具证明。另制造提交返回丢失但页面保留唯一匹配 user turn 的自动恢复、零个或多个候选的阻塞、携带正确或错误 canonical URL 的 `submitted` 裁决，以及用户给出 `not-submitted` 但页面分别处于安全 composer、有匹配提交或错误 Project 的场景。最后在 live task 启动长 response，用户明确将其裁决为失败；分别在 Stop 已消失和仍可见时执行 `resolve-turn ... failed`，再显式发送 continuation。另对包含已发布 response、completed artifact 与带错误 pending artifact 的 capturing turn 执行同一命令。确定性页面夹具覆盖 browser 缺失重建、目标或 conversation 不符、后续 user turn、composer 不安全、Stop 结束失败、重复与并发裁决。需要页面检查的场景记录 browser command、页面 identity、operation transition、task/turn transition 和 user turn 数量；capturing 裁决只记录本地 turn 与 artifact 前后状态。
- **通过证据**：`status` 前后数据库与页面无变化；active task 的 browser 缺失与 closed task 均返回 `recover`。未绑定 task 只恢复同一 `taskId` 的固定 Project composer，已绑定 task 只恢复同一 canonical conversation 与相关 turn，两个并发 `recover` 只创建一个有效 named session；closed task 只在这些验证后改回 active，completed turn 后的 feedback 保留首轮上下文（真实 Web 验证）；closed task 的 pending turn 恢复后继续原 wait，由确定性 State/Browser 夹具证明。唯一页面匹配可自动置为 `pending` 且 user turn 总数不增加；无 canonical identity、零个或多个候选进入 `needs-decision` 且不发送。有效 `submitted` URL 只在固定 Project、既有 task 绑定、conversation、保存的 prompt、有序附件名称和唯一 user turn 全部匹配后绑定原 turn；有效 `not-submitted` 只在用户裁决且已绑定 conversation 或未绑定 Project composer 验证安全后把原 turn 置为 `failed`。有效 response 失败裁决作用于原 active pending 或 capturing turn；pending 必要时至多点击一次目标 Stop，capturing 不读取或操作页面、不新增页面证明，并保持已发布 response 字节、completed artifact、pending artifact 及其错误不变。两者都不创建 user turn；裁决后的只读 session 探测与当前持久状态共同决定 `send`、`wait`、`recover` 或 `none`，显式 continuation 保持相同 canonical conversation 并使用此前上下文。错误 URL、输入不符、候选不唯一、有匹配提交、错误 Project、无权限、错误 turn 状态、页面漂移和并发状态冲突均保持原状态；重复 response 裁决不再点击或写消息；所有路径的自动或人工 resolution source 或 turn error 可审计，消息均不自动重发。
- **证明边界**：页面匹配只能证明当时可见的 conversation、prompt 和附件名称，不能证明远端附件字节；`not-submitted` 与 response 失败的历史事实来自用户裁决，页面只证明后续发送状态安全；受控点击 Stop 证明终止与 continuation 路径，不证明 ChatGPT Web 每种自然失败提示；不证明已删除或当前账号无权访问的 conversation 可恢复。
- **必需性**：必需。
...
| 陈旧页面按单调时间周期 reload       | 已确认 | 服务端已完成但 Web 客户端停留在不可捕获状态时，普通 DOM 信号（Stop、Copy、Submit、文本）无法与“仍在生成”区分；只依赖单调时间的周期 reload 重新水合同一 canonical conversation 后走既有捕获路径，不点击 Stop、不自动失败、不发送 continuation，reload 耗时计入原观察窗口只在剩余预算内继续 | BEH-004、Playwright CLI 合同、VER-004                                                           |
...
| close 与 Web archive 分离           | 已确认 | 本地进程生命周期与 Web conversation 生命周期是两种独立副作用；close 只把 task 置为可恢复暂停态，后续 feedback 可经 recover 继续原 canonical conversation，Web archive 仍须由独立显式命令触发                                                                                              | BEH-005、BEH-008、BEH-009、BEH-013、VER-008、VER-009、VER-016                                   |
| task daemon PID 定向清理            | 已确认 | 固定 CLI 的 named-session 记录可能先于 detached daemon 与 headed Chrome 退出而消失；持久化 task-owned PID 并在 session 身份可证明时终止对应进程组，可清理 macOS Dock 残留且不影响其他 task 或外部 Playwright session                                                                      | BEH-007、BEH-008、SQLite 合同、VER-007、VER-008                                                 |
...
| 用户裁决失败 response 后显式续写    | 已确认 | response 终端失败是用户提供的历史事实，不能由 pending 或超时推断；先验证原 conversation 与安全 composer、裁决旧 turn，再由宿主显式发送具体 continuation，才能保持上下文且不制造重复消息                                                                                                   | BEH-004、BEH-005、BEH-007、BEH-013、命令合同、VER-005、VER-007、VER-016                         |
...
| 提交前中断的 clean-or-rebuild 恢复  | 已确认 | 页面内 reload 能清除并核对普通附件草稿；无法证明 composer 安全时，`close` 收口同名 session，再在既有 task 身份约束内重建可验证安全 composer，保持恢复自动化且绝不自动重发                                                                                                                 | BEH-003、BEH-013、Playwright CLI 合同、浏览器副作用恢复、VER-003                                |
| 页面内附件注入与可见就绪证明        | 已确认 | 当前页面不再稳定提供可跨 CLI 命令消费的 fileChooser modal；同一 `run-code` 对隐藏 file input 执行 `setInputFiles`，并等待 composer 显示累计有序附件名称，避免把文件选择动作本身误当成业务上传已就绪                                                                                       | BEH-003、Playwright CLI 合同、VER-003                                                           |
