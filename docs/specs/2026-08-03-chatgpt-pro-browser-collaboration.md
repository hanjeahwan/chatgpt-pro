...
`status` 不取得 mutating operation lease，也不执行本表动作；它对同一数据库快照返回 `taskId`、`taskStatus`、可空 `turnId`、可空 `turnStatus`、`browserStatus: available | missing | unknown`、可空 `operationKind`、可空 `operationStep`、可空 `operationPhase`、可空 `operationProgress`、最近 evidence、最近 error 和唯一 `nextAction`。`nextAction` 表示继续或解除当前持久工作流的系统选定动作，不枚举依赖用户新事实的 `resolve-turn` 等主动命令；没有待继续或待解除状态时为 `none`。`closing` 优先返回 `close`；closed task 或 active task 的 browser 确认缺失时优先返回 `recover`；恢复成功后再按 turn 返回 `wait` 或 `none`。`recover` 取得 lease 后只执行表中与当前持久状态匹配的一个恢复路径，并返回恢复后的同一组状态字段；若下一步是 `wait`、`close` 或人工提交裁决，只返回该 `nextAction`，不代替对应命令。
