...
| send operation 的 step 为 `draft`                 | 先页面内清理目标 composer；被挂起 file chooser（modal state）阻塞、无法证明安全时，关闭同名 named session 并从 seed 重建：已绑定 conversation 的任务恢复精确 canonical conversation，未绑定的任务恢复固定 Project 空白 composer；重建并验证安全后把原 turn 置为 `failed` 并提交 operation，返回 `nextAction: send` | 不继续上传，不提交消息，不保留残留或重复附件、不残留挂起 chooser；新的消息必须由宿主显式再次 `send`；重建不迁移或猜测 conversation |
...
- **执行或检查**：宿主先把 Skill 中的固定协作合同与当前任务写入同一 prompt 文件，再从工作区内外各选择明确的普通附件执行首次 `send`，记录输入选择层的路径解析、文件访问和读取调用；另以 live Web 检查 user turn 文字与实际上传项，不传入其他文件。分别在每个附件上传后且提交 command 放行前终止进程，再执行 `recover` 并重新进入显式 send 流程。对自然 kill 后残留挂起 file chooser 的中断点，用固定 CLI 复现 `run-code`/`reload` 被 modal guard 拒绝与 `upload` 无法表达取消的确定性，并实机复验该子状态：页面内清理失败时实际执行一次关闭同名 session 并从 seed 重建的恢复（已绑定与未绑定任务各一），再进入显式 send 流程。另用确定性可执行夹具覆盖上传准备的 modal-handoff：准备 run 因 fileChooser modal 竞速返回空结果后分别出现 (a) 立即 upload 成功、(b) 立即 upload 工具错误、以及 (c) 空结果后准备或清理输出为非空非 envelope 的畸形或不匹配 envelope；记录每次准备与 upload 调用的 stdout、错误分类与是否进入 submit。
- **通过证据**：首个 user turn 与指定 prompt 文件逐字一致，在一条消息内完整包含一份固定协作合同和当前任务，且没有独立启动 user turn。Pro 的第一个回复直接处理当前任务，或在真正阻塞时提出一个聚焦问题，不只确认协作模式。输入选择层只解析和访问指定 prompt 与附件；显式外部路径可上传；Web 没有未指定上传项或归档；正常路径返回 turnId。提交放行前的每个中断点都恢复为没有残留或重复附件的安全 composer，原 turn 标为 `failed`，未产生 user turn，也未自动发送；新的消息只有在宿主再次显式 `send` 后出现，且其 prompt 仍包含完整协作合同。残留挂起 file chooser 的中断点恢复后没有残留或重复附件也没有挂起 chooser：页面内清理或关闭重建均返回经页面验证的安全 composer，已绑定任务恢复精确 canonical conversation，未绑定任务恢复固定 Project 空白 composer，原 turn 标为 `failed`，未产生 user turn，也未自动发送。上传准备空结果（modal handoff）后立即 upload 成功时恰好一次 submit；空结果后 upload 工具错误时不提交、调用 clean-or-rebuild 且宿主错误保留具体固定 CLI 错误文本；非空非 envelope 或畸形不匹配 envelope 保持合同漂移；cleanup 成功、cleanup 失败与结构化 CLI 工具错误可区分；一次真实显式附件发送恰好产生一个 user turn 且附件到达。
- **证明边界**：不约束 Node.js、npm/npx、浏览器或操作系统读取其运行依赖、配置与缓存；不证明归档输入或 Pro 回复正确；chooser 悬挂、modal guard 与 modal-handoff 竞速的确定性以固定 CLI 版本行为为界，不做跨版本推断。
- **必需性**：必需。
...
| 提交前中断的 clean-or-rebuild 恢复  | 已确认 | 固定 CLI 无法表达取消挂起 chooser、modal guard 拒绝页面内清理工具，而 `close` 是 session 命令可收口会话；关闭重建能在既有 task 身份约束内取得可验证安全 composer，保持恢复自动化且绝不自动重发                                                                                            | BEH-003、BEH-013、Playwright CLI 合同、浏览器副作用恢复、VER-003                                |
| file chooser 移交的暂定就绪语义     | 已确认 | 固定 CLI 的 `run-code` 与 fileChooser modal 竞速时会在页面函数完成前提前返回空结果；把它当失败会误伤正常路径，把它当成功又无法证明，只有紧接着的 `upload` 命令（固定 CLI 中消费并清除该 chooser 的既定操作）能证明就绪，以证明代替重试或新状态机，保持恢复合同最小                        | BEH-003、Playwright CLI 合同、VER-003                                                           |
