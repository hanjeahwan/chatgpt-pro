...
## 待查一：长会话下锚点被虚拟化，提交校验与裁决必然失败

### 症状

会话累积到十余轮后，`send` 稳定返回 `SUBMISSION_UNKNOWN`，`resolve-submission` 稳定在 `page.waitForFunction: Timeout 60000ms exceeded` 失败。两者都无法恢复，`recover` 无效，任务被卡住无法继续发送。

消息本身**每次都真的发出去了**：用独立浏览器读同一会话，能看到带附件的 user 消息与其后的助手回复。失败的只是本地校验与记账。

### 实测数据

同一会话在不同阶段观测 `document.querySelectorAll('[data-testid^="conversation-turn-"][data-turn]')`：

| 会话规模 | DOM 中的 turn 数 | 已存在的 turn 编号                             |
| -------- | ---------------: | ---------------------------------------------- |
| 4 轮     |                7 | 1、3、4、5、6、7、8                            |
| 8 轮     |                7 | 1、3、6、7、8、9、10（**2、4、5 已不在 DOM**） |
| 12 轮    |                0 | 生成期间整体缺失                               |

也就是说 ChatGPT 会把较早的 turn 从 DOM 移除，且移除的不只是最早几条。

### 根因（推断，未验证到代码级）

两处都要求锚点当前可见：

- `browser.ts` 的 `autoVerifySubmission` 与 `resolveSubmittedScript` 都先 `waitForFunction` 等 `previousUserTurnIdentity` 出现，超时 60000ms。
- 锚点是上一轮成功 turn 的 `conversation-turn-N`。随着会话增长，这个编号先于被验证的新 turn 被虚拟化移出。

因此等待必然耗尽 60 秒，而被验证的新 turn 其实就在页面上。

`ISSUE.md` 里的第 5 项修复（settle 到证据可读）解决的是「新 turn 尚未渲染」，与本项相反——本项是**旧锚点已被移除**，settle 再久也等不到。

### 建议方向（未实现）

锚点不应要求当前可见。可考虑：把锚点降级为「若可见则用于定位，不可见则回退到全页唯一性判定」；或改用不依赖相对位置的标识（例如按 prompt 证据在全部可见 user turn 中唯一匹配）；或在等待前主动向上滚动触发历史 turn 渲染。

### 当前绕法

直接改本地 `state.sqlite`：把卡住的 turn 置 `failed`，并把其 `needs-decision` 的 operation 置 `committed`。这不触碰 ChatGPT 会话，`task.conversation_id` 保持不变，下一次 `send` 正常进入同一会话。这是绕过而非修复。
...
六个已修根因里有四个、两个待查项全部是同一类：**把「页面上读到的东西」直接当成事实，而没有区分「尚未渲染」「已被移除」「渲染方式不同」与「确实不存在」**。ChatGPT 的 DOM 在这四方面都会变化，而当前实现大量依赖 `getClientRects()` 判可见、依赖 `textContent` 逐字比对、依赖 `data-testid` 唯一性与相对位置。

两个待查项在时间轴上正好相反：待查一是**旧锚点已被移除**，待查二是**目标元素在整页最长处**。任何「先等某个 testid 出现再读」的写法都会同时受这两端影响，因此建议把定位策略从「相对锚点 + 当前可见」改为「内容自证 + 全页唯一」。
